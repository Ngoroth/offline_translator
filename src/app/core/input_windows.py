...
class KeyboardInput(InputProvider):
    ...
    def _on_press(self, key):
        for id, target in self._key_map.items():
            if self._is_key_match(key, target):
                if not self._states[id]:
                    self._states[id] = True
                    self._release_events[id].clear()
                    self._press_queue.put(id)

    def _on_release(self, key):
        for id, target in self._key_map.items():
            if self._is_key_match(key, target):
                self._states[id] = False
                self._release_events[id].set()

    def _is_key_match(self, key, target_name):
        ...
        if target_name == "alt" and (
            ...
            return True
        if hasattr(key, "char") and key.char == target_name:
            return True
        return False
    ...
    def wait_for_release(self, input_id: str) -> None:
        if input_id in self._release_events:
            self._release_events[input_id].wait()
