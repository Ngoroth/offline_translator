...
class AudioRecorder:
    ...
    _buffer: list[np.ndarray[tuple[int], np.dtype[np.float32]]]
    ...
    def _callback(
        self,
        indata: np.ndarray[tuple[int, int], np.dtype[np.float32]],
        _frames: int,
        _time: object,
    ...
    def get_last_chunk(self, num_samples: int) -> np.ndarray[tuple[int], np.dtype[np.float32]]:
    ...
    def extract_buffer(self) -> np.ndarray[tuple[int], np.dtype[np.float32]]:
    ...
    def stop(self) -> np.ndarray[tuple[int], np.dtype[np.float32]]:
        if self._stream:
            self._stream.stop()
            self._stream.close()
            self._stream = None
